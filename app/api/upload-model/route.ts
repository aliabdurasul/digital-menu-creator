...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("[/api/upload-model] FULL ERROR:", JSON.stringify(err, Object.getOwnPropertyNames(err instanceof Error ? err : {})), err);
    let message: string;
    if (err instanceof Error) {
      ...
      message = JSON.stringify(err) || "Model upload failed (unknown)";
    }
    return NextResponse.json(
      { error: message, step: "upload-model" },
      { status: 500 }
    );
  }
}
