 ...
 * session_id. When the order is marked "ready", it:
 *   1. Shows a fixed full-width banner at the top of the page
 *   2. Fires a browser Notification (if permission was granted)
 *   3. Plays a short beep sound
 *
...
export function OrderReadyWatcher({ moduleType }: { moduleType?: "cafe" | "restaurant" }) {
  ...
  if (!orderReady || dismissed) return null;
...
function fireNotification(sessionCode: string) {
  try {
