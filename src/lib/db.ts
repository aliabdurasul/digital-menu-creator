    ...
    const { data: dbRestaurant, error } = await supabase
      ...
      .select(`
        *,
        ...
        menu_items ( id, restaurant_id, category_id, name, description, price, image_url, is_available, "order", ingredients, portion_info, allergen_info, ar_model_url, ar_model_size_cm, created_at, updated_at )
      `)
      .eq("slug", slug)
      .single();
